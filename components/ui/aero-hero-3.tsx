...
export const Component = () => {
  return (
    <section className="aero-hero relative flex min-h-[calc(100svh-72px)] w-full items-center justify-center overflow-hidden">
      <div
        className="aero-hero-grid absolute inset-0 z-10 size-full"
        aria-hidden="true"
      >
      ...
      <div className="relative z-30 mx-auto flex w-full max-w-6xl flex-col items-center justify-center px-6 text-center text-white">
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[11px] font-medium tracking-[.14em] text-white/85 uppercase shadow-[0_8px_30px_rgba(0,0,0,.12)] backdrop-blur-xl">
          <Leaf className="size-3.5 text-[#e1fcad]" /> AI-powered carbon
          intelligence
        </div>
        ...
        <h1 className="aero-hero-title max-w-4xl text-balance font-normal leading-[.92] tracking-[-.065em] text-white text-4xl sm:text-6xl md:text-8xl">
          Carbon intelligence, measured in every{" "}
          <span className="text-[#e1fcad]">snap.</span>
        </h1>

        <p className="mt-8 max-w-2xl text-balance text-base font-light leading-7 text-white/82 md:text-xl md:leading-8">
          Snap2Green turns bills, receipts and travel records into transparent
          carbon insights so people and businesses can understand their
          footprint and take meaningful action.
        </p>

        <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link
            href="/dashboard"
            aria-label="Open Snap2Green dashboard"
            className="group flex h-auto cursor-pointer items-center justify-center gap-0 rounded-full"
          >
            <span className="rounded-full bg-[#e1fcad] px-6 py-3.5 font-medium text-[#102218] duration-500 ease-in-out group-hover:bg-[#122023] group-hover:text-[#e1fcad]">
              Scan your footprint
            </span>
          ...
          <Link
            href="/features"
            className="rounded-full border border-white/20 bg-white/8 px-6 py-3.5 text-sm text-white/90 backdrop-blur-xl transition hover:bg-white/14 hover:text-white"
          >
            Explore the system
          </Link>
        </div>
        ...
        <a
          href="#parallax-story"
          className="mt-16 flex w-fit items-center gap-2 text-xs text-white/55 transition hover:text-white/90"
          aria-label="Scroll to how Snap2Green works"
        >
